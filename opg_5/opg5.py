...
def Area(time):
    if get_stage(time) == 0 or get_stage(time) == 1 or get_stage(time == 2):
...
def Fsum(time, height, velocity):
    return get_thrust(time) - rocket_gravity(height, estimate_mass(time)) - air_resistance(height, time, velocity)


def acceleration(time, height, velocity):
    return Fsum(time, height, velocity) / estimate_mass(time)


def rocket_velocity_change(time):
    return get_exhaust_velocity(time) * math.log((total_mass[0] / estimate_mass(time)), math.e)
